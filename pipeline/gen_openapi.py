...
"""Generate contract/openapi.json — the OpenAPI 3.1 description of every /api/* route.
    ...
    python3 pipeline/gen_openapi.py [--check]
    ...
    return {
        ...
        "info": {
            ...
            "description": ("Read-only API over the verbatim Sri Guru Granth Sahib Ji corpus (60,658 lines, 1,430 Angs). "
                            ...
                            "Generated by pipeline/gen_openapi.py; response schemas are inferred from real responses."),
        },
...
def main(argv):
    ...
    if "--check" in argv:
        if not OUT.exists() or OUT.read_text(encoding="utf-8") != text:
            print("contract/openapi.json is stale — run: python3 pipeline/gen_openapi.py")
            return 1
