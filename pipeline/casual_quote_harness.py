...
Runs do_search IN-PROCESS (no HTTP). Usage:
  python3 pipeline/casual_quote_harness.py [--sample N] [--seed S] [--words W] [--show-fails K]
"""
...
def main():
    ...
    ap.add_argument('--out', default=os.path.join(ROOT, 'validation', 'casual_quote_results.jsonl'))
    args = ap.parse_args()
