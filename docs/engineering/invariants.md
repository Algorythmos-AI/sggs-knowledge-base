...
- **`roman_norm` exists in three places** — `webapp/romannorm.py` (query time), sggs-data's `pipeline/sggs_pipeline.py` (index time) and the app's `RomanNorm.swift` (the port) — and **must stay byte-identical**, or search silently breaks. `contract/golden_roman_norm.ndjson` (24,719 vectors, checked against the indexed column and vendored by the app) proves it; a change to the fold is a coordinated dataset release. See [The Roman fold](../search/the-roman-fold.md).
