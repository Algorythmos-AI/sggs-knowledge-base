...
**Environments:** `integration` = trunk → auto-deploys **staging** (`sggs-staging.vercel.app`, SSO-protected; the API runs as Vercel functions inside it). `main` = **production** (web on the canonical domain **`gurbanisoul.com`**; `www` and the legacy `*.vercel.app` alias 308→apex; API `…onrender.com`). Both deploy **only** through CI (`deploy-staging.yml` / `deploy-production.yml`); the platforms' own git auto-deploys are OFF and Render auto-deploy is OFF. Never deploy by hand except a documented pipeline-outage hotfix.
