...
edition, Sant Lipi is SIL OFL 1.1, and SQLite is public domain.
...
4. **Scholar review is a gate, not a nicety.** Two items in the readiness report are pending it:
   the placeholder app icon (ੴ) and the default-ON saroop rendering (Sant Lipi's inline addha-yayya
