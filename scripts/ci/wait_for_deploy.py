...
def main():
    ...
    a = ap.parse_args()
    url = a.base_url.rstrip("/") + "/api/health"
    want = a.commit.strip()
    ...
    while True:
        ...
        try:
            ...
            if got == want:
                if h.get("ok") is True and all(h.get("checks", {}).values()):
                    print(f"  [{elapsed:>4}s] {state} — deployed and healthy"); return 0
                print(f"::error::commit {want[:12]} is live but health failed: {h.get('checks')}"); return 1
